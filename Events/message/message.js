...
class message extends Event {
    ...
    async run( message ) {
        ...
        message.guild && new client.db.Message({
            _ID         : message.id,
            _userName	: message.author.username,
        ...
        const prefix = message.guild ? message.guild.prefix : "k!";
        ...
        // —— Chatbot
  		if (message.channel.id === "777134013386063883") {
    		...
    		fetch(`https://api.monkedev.com/fun/chat?msg=${encodeURIComponent(content)}&key=${client.config.monkeDevAPI}`)
      		.then(res => res.json())
      		.then(json => message.inlineReplyNoMention(json.response))
      		.catch(console.error);
